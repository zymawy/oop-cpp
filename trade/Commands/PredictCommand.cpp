...
void PredictCommand::run() {
    ...
    // we will return in favor of early return concept!
    if (arguments.size() < 4) {
    ...
    std::string maxOrMin = arguments[1];
    std::string product = arguments[2];
    std::string type = arguments[3];
    ...
    if ((maxOrMin != "min" && maxOrMin != "max")) {
        ...
        throw std::runtime_error(
                std::string(
                        "argument missing/invalid maxOrMin, it should be ether min or max you entered [" + maxOrMin +
                        "]")
        );
    }
    ...
    std::map<std::string, double> bidOrAsksAvg{gatherTransactions().at(OrderBookEntry::convertType(type))};
    std::string availableSuggestionProductsName;
    // let's get going and see if we have product in
    // this current time with current entry type ...
    for (auto &e: bidOrAsksAvg) {
        if (!UserInputProcessor::is_number(e.first)) {
            availableSuggestionProductsName =
                    availableSuggestionProductsName + (availableSuggestionProductsName.length() ? "," : "") + e.first;
        }
    }
    ...
    try {
        ...
        std::string errorMessage =
                "There is not enough data to predict the future based on the current timestamp: " + currentTime +
                " and current product " + product;
        if (availableSuggestionProductsName.length()) {
            errorMessage = "There is not enough data to predict \n we suggest try those products " +
                           availableSuggestionProductsName;
        }
    ...
    UserInputProcessor::print(
            " The average " + product + " " + type + " price over the last 10 timestamps was "
            + std::to_string(predictAvg)
    );
}

std::map <OrderBookType, std::map<std::string, double>> PredictCommand::gatherTransactions() {
    ...
    for (const std::string &product: orderBook.products) {
        try {
            ...
            if (orderBook.orders.find(currentPreTime) != orderBook.orders.begin() && orders.size() > 0) {
                ...
                if (calculateSmoothAverage(OrderBookType::ask, product, 10, currentPreTime) <
                ...
                if (calculateSmoothAverage(OrderBookType::bid, product, 10, currentPreTime) >
...
double
PredictCommand::calculateSmoothAverage(const OrderBookType &type, const std::string &product, const int &timeframes,
